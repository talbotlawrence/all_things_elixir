...
  # Functions start with def, the functions name and then end
  def main do

    # gets retrieves user input trim removes the newline
    name = IO.gets("What is your name? ")
    |> String.trim

    # You can combine string and variable output with #{}
    IO.puts "Hello #{name}"
    data_stuff()
  end

  # ----- DATA TYPES -----
  # Variables start with lowercase letters or underscores and then uppercase letters, underscores or numbers
  # Once set a variables value can't change Types Integers, Floats, Atoms, Ranges, Binaries, Lists, Maps, Tuples
  def data_stuff do
...
    # You can combine strings--concatenate
...
    # Split a string into a list; Inspect which prints the internal representation of a value
...
    # Cond works like else if it only hits the 1st match
    cond do
...
    # ----- TUPLES -----
    # Tuples normally have 2 to 4 values, They can hold any value, They aren't for enumerating or cycling through like lists, They are for pattern matching
...
    # Create a tuple with the same values duplicated a number of times
...
    # Inspect which prints the internal representation of a value may give odd results
    IO.inspect [97,98]

    # You can force inspect to print as a list like this
...
    # ----- PATTERN MATCHING -----
    # With pattern matching we can retrieve data from data structures
...
    # ----- ANONYMOUS FUNCTIONS -----
    # An anonymous function has no name and can be passed to another function
    get_sum = fn (x, y) -> x + y end
    IO.puts "5 + 5 = #{get_sum.(5,5)}"

    # Shorthand of th above Increment the parameter names by 1
    get_less = &(&1 - &2)
    IO.puts "7 - 6 = #{get_less.(7,6)}"

    # You can have functions except different numbers of parameters
    add_sum = fn
      {x,y} -> IO.puts "#{x} + #{y} = #{x+y}"
...
    # ----- LOOPING -----
    # With most languages we loop by incrementing or decrementing a value
    # Since all variables are immutable in Elixir we use recursion to loop
...
    # ----- ENUMERABLES -----
    # An enumerable can have its items counted off
...
    # ----- LIST COMPREHENSIONS -----
    # Provides an additonal way to perform actions on a list
...
    # ----- CONCURRENCY -----
    # Running multiple blocks of code at once; Calling spawn starts a new process that runs the code specified at an undefined time
...
    # Send a message to this process and receive it
...
    # If there is no matching message you can issue a timeout like I did for 500 milliseconds
    after
...
    # This will end the calling of additional functions
    if num <= 1 do
      1
    else
...
  # Demonstrate looping Sum values in a list
...
  # A Elixir for loop What happens if the 1st attribute is 0
  def loop(0,_), do: nil
